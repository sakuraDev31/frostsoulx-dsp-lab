...
object DiagnosticReport {
    ...
    fun build(
        ...
        line("Quantum FIFO underflow", if (t.fifoUnderflowFrames > 0)
            "${t.fifoUnderflowFrames} frames (%.1f ms)".format(t.fifoUnderflowMs) else "0 frames")
        line("Sink underruns", t.sinkUnderruns)
        line("Last sink underrun gap", if (t.sinkUnderrunMs >= 0)
            "%.0f ms without a write".format(t.sinkUnderrunMs) else null)
        line("Sink errors", t.sinkErrors)
        line("AudioTrack buffer", t.sinkBufferMs?.let { "${t.sinkBufferFrames} frames (%.1f ms)".format(it) })
